...
func (s *ModuleServer) Run() error {
	...
	m.RegisterModule(modules.StorageServer, func() (services.Service, error) {
		...
		return sql.ProvideUnifiedStorageGrpcService(s.storageBackend, s.cfg, s.features, nil, s.log, s.registerer, docBuilders, s.storageMetrics, s.indexMetrics, s.searchServerRing, s.MemberlistKVConfig)
	})
